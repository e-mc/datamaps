(function() {
  ...
  var defaultOptions = {
    ...
    geographyConfig: {
        ...
        countryLabelTemplate: function(geography, data) {
          return '<div class="countrylabel"><strong>' + geography.properties.name + '</strong></div>';
        },
        popupOnHover: true,
        titleOnHover: false,
        footerOnHover: false,
    ...
    if ( options.highlightOnHover || options.popupOnHover ) {
      svg.selectAll('.datamaps-subunit')
        .on('mouseover', function(d) {
          ...
          if ( options.titleOnHover || options.footerOnHover ) {
            self.updateCountryLabel($this, d, options, svg);
          }
        })
  ...
  Datamap.prototype.draw = function() {
    ...
    return this;
      ...
      function draw (data) {
        ...
        if ( self.options.geographyConfig.titleOnHover ) {
          countryTitle = d3.select( self.options.element ).insert("div",":first-child");
              .attr('class', 'datamaps-countrylabel')
        }
        if ( self.options.geographyConfig.footerOnHover ) {
          countryTitle = d3.select( self.options.element ).append('div')
              .attr('class', 'datamaps-countrylabel')
        }
  ...
  ***************************************/
  ...
  Datamap.prototype.updateCountryLabel = function (element, d, options) {
    var self = this;
    element.on('mousemove', null);
    element.on('mousemove', function() {
      var position = d3.mouse(self.options.element);
      d3.select(self.svg[0][0].parentNode).select('.datamaps-countrylabel')
          .html(function() {
            var data = JSON.parse(element.attr('data-info'));
            try {
              return options.countryLabelTemplate(d, data);
            } catch (e) {
              return "";
            }
          });
    ...
    d3.select(self.svg[0][0].parentNode).select('.datamaps-countrylabel').style('display', 'block');
  };
